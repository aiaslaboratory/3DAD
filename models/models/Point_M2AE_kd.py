    ...
    def forward(self, pts, eval=False, **kwargs):
        ...
        for i in range(len(self.decoder_dims)):
            ...
            if i == 0:
                ...
                #print(f"Before h_decoder[{i}] - x_full size: {x_full.size()}")
                B, _, C = x_full.shape
                ...
                #print(f"Before h_decoder[{i}] - pos_emd_vis size: {pos_emd_vis.size()}")
                #print(f"Before h_decoder[{i}] - pos_emd_mask size: {pos_emd_mask.size()}")
                
                
                pos_full = torch.cat([pos_emd_vis, pos_emd_mask], dim=1)
                #print(f"Before h_decoder[{i}] - pos_full size: {pos_full.size()}")
                ...
                #print(f"Before h_decoder[{i}] - x_full size: {x_full.size()}, pos_full size: {pos_full.size()}")
            #print(f"Before h_decoder[{i}] - x_full size: {x_full.size()}, pos_full size: {pos_full.size()}")
        ...
        num_points_per_center = 16  # 每个中心点对应的重构点数量
        ...
        return loss,original_points,gt_points,rec_points,target_layer_points,center_points,mas_points,absolute_rec_points,dismas_points
