...
class H_Encoder(nn.Module):
    ...
    def _forward_pass(self, neighborhoods, centers, idxs, bool_masked_pos):
        # generate mask at the highest level
        bool_masked_pos_copy = [x.clone() for x in bool_masked_pos]
        ...
        for i in range(len(neighborhoods) - 1, 0, -1):
            ...
            idx_masked = ~(bool_masked_pos_copy[-1].reshape(-1).unsqueeze(-1)) * idx
            idx_masked = idx_masked.reshape(-1).long()
            
            masked_pos = torch.ones(b * centers[i - 1].shape[1]).cuda().scatter(0, idx_masked, 0).bool()
            
            bool_masked_pos_copy.append(masked_pos.reshape(b, centers[i - 1].shape[1]))
           
            
            
            mask_sum = masked_pos.long().sum().item()
            
        #self.saved_masks = bool_masked_pos
        #self.saved_unmasked_points = [centers[i][~m] for i, m in enumerate(bool_masked_pos)]
        # hierarchical encoding
        bool_masked_pos_copy.reverse()
        x_vis_list = []
        mask_vis_list = []
        xyz_dist = None
        for i in range(len(centers)):
            ...
            # visible_index
            bool_vis_pos = ~(bool_masked_pos_copy[i])
        ...
        return x_vis_list, mask_vis_list, bool_masked_pos_copy
    ...
    def forward(self, neighborhoods, centers, idxs, eval=False):
        ...
        if eval:
            ...
            result = self._forward_pass(neighborhoods, centers, idxs, bool_masked_pos)
            ...
            test_bool_masked_pos = [self.rand_mask(centers[-1])]
            _, test_mask_vis_list, _ = self._forward_pass(neighborhoods, centers, idxs, test_bool_masked_pos)

            for level, mask in enumerate(test_mask_vis_list):
                print(f"Level {level}: visible points = {mask.sum()}")
            ...
            bool_masked_pos1 = [~x for x in bool_masked_pos]
  
            result = self._forward_pass(neighborhoods, centers, idxs, bool_masked_pos1)
            results.append(result)
...
class Point_M2AE_test(nn.Module):
    ...
    def __init__(self, config):
        ...
        # prediction head
        self.rec_head = nn.Conv1d(self.decoder_dims[-1], 3 * self.group_sizes[1], 1)
        # loss
        self.rec_loss = ChamferDistanceL2().cuda()

    def decode_and_rec(self,pts, x_vis_list, mask_vis_list, masks, centers, neighborhoods,mas):
        ...
        original_points = pts
        target_layer_points = neighborhoods[-2]
        center_points = centers[-1]
        mas_points = mas[-2]
        gt_points = neighborhoods[-2][masks[-2]].reshape(L, -1, 3)
        dismas_points =  mas_points[~masks[-2]]
        mas_points = mas_points[masks[-2]].reshape(L, -1, 3)
        # CD loss
        loss = self.rec_loss(rec_points, gt_points)
        masked_centers = centers[1][masks[1]]
        num_points_per_center = 8  # 每个中心点对应的重构点数量
        expanded_masked_centers = masked_centers.unsqueeze(1).repeat(1, num_points_per_center, 1)
       # 假设 masks 是一个包含多层布尔型张量的列表
        mask_counts = [mask_layer.sum().item() for mask_layer in masks]
        # 计算绝对坐标
        absolute_rec_points = rec_points + expanded_masked_centers
 
        
        return loss,original_points,mas_points,absolute_rec_points,dismas_points 
    def forward(self, pts, eval=False, **kwargs):
        ...
        if eval:
            ...
            results = self.h_encoder(neighborhoods, centers, idxs)
            (x_vis_list1, mask_vis_list1, masks1),(x_vis_list2, mask_vis_list2, masks2) = results
        ...
        outputs1 = self.decode_and_rec(pts,x_vis_list1, mask_vis_list1, masks1, centers, neighborhoods,mas)
        
        outputs2 = self.decode_and_rec(pts,x_vis_list2, mask_vis_list2, masks2, centers, neighborhoods,mas)
        (loss1,_,_,_,_) = outputs1
        (loss2,_,_,_,_) = outputs2
        loss = loss1 + loss2
        # 返回两组输出
        return loss,outputs1, outputs2
